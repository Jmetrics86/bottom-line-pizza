...
// Buying actions
(window as any).buyStaff = function(itemKey: string): void {
...
(window as any).buyUpgrade = function(upgKey: string): void {
...
function init(): void {
    ...
    if (staffContainer) {
        staffContainer.addEventListener('click', (e: MouseEvent) => {
            const btn = (e.target as HTMLElement).closest('.buy-btn');
            if (btn) {
                const key = btn.getAttribute('data-key');
                if (key) {
                    (window as any).buyStaff(key);
                }
            }
        });
    }
    ...
    if (upgradesContainer) {
        upgradesContainer.addEventListener('click', (e: MouseEvent) => {
            const btn = (e.target as HTMLElement).closest('.buy-btn');
            if (btn) {
                const key = btn.getAttribute('data-key');
                if (key) {
                    (window as any).buyUpgrade(key);
                }
            }
        });
    }
